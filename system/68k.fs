...
: imm#, ( n -- )
    opsize @ case
        1 of ?char not imm-error $FF and asmh, endof
        2 of ?half not imm-error         asmh, endof
        4 of ?cell not imm-error         asm,  endof
...
: <ext> ( ext ea n -- ext ea ) third @ swap 1 = if asm, exit endif asmh, ;
: (ext) ( ext ea -- )
    dup @ $38 and case
        $28 of over @ asmh, endof
        $30 of over @ asmh, endof
...
:  ea,  ( n -- ) ea @ + opsize+ asmh, ext1, ;
: imm,  ( n opcode -- ) %111100 + asmh, imm#, ;
...
: (string) ( arg1 arg2 addr -- )
    @ >r 2arg case
        dd' of sdreg r> + asmh, endof
        mm' of 2drop --? not effect-error  (easd)  r> + 8 + asmh, endof
...
: (single) ( n opcode -- ) >r 1arg case
        d' of sreg r> + opsize+ asmh, endof
        m' of drop r> ea, endof
    invalid endcase clean ;
: set?, ( arg cc -- ) <byte> $50C0 +cc (single) ;
: nbcd,    ( arg -- ) <byte> $4800     (single) ;
: test,    ( arg -- )
    1arg a' = if 15 and $4A00 + opsize+ asmh, clean exit endif $4A00 (single) ;
...
: (data) ( arg1 arg2 opcode -- )
    >r 2arg case
        dd' of sdreg r> (and1) opsize+ asmh, endof
        dm' of drop dreg $100 + r> (and1) ea, endof
        #d' of  nip sreg r> (and2) opsize+ asmh, imm#, endof
        #m' of 2drop r> $0F00 and ea @ + opsize+ asmh, imm#, ext1, endof
    invalid endcase clean ;
...
: (quick) ( n arg opcode -- )
    >r 1arg case
        d' of swap sdreg r> (and1) opsize+ asmh, endof
        a' of swap adreg r> (and1) opsize+ asmh, endof
...
: arithmetic: ( opcodes "name" -- ) create swap , , does> ( arg1 arg2 -- )
    ...
    @ >r 2arg case
        da' of nobyte sdreg r> (and1) opmode+ asmh, endof
        ad' of nobyte adreg r> (and1) opsize+ asmh, endof
        aa' of nobyte adreg r> (and1) opmode+ $4 + asmh, endof
...
: shift: ( opcode "name" -- ) create , does> ( arg1 arg2 -- )
    ...
    3 lshift + >r 2arg case
        dd' of swap sdreg $20 + r> + opsize+ asmh, endof
        #d' of nip over 1 9 within not imm-error
               swap sdreg r> + opsize+ asmh, endof
...
: bit: ( opcode "name" -- ) create , does> ( arg1 arg2 -- )
    @ >r 2arg case
        dd' of <long> swap sdreg $100 + r> + asmh, endof
        dm' of <byte> drop  dreg $100 + r> + ea, endof
        #d' of <long>  nip  sreg $800 + r> + asmh, asmh, endof
        #m' of <byte> 2drop $800 r> + ea @ + opsize+ asmh, asmh, ext1, endof
    invalid endcase clean ;
...
: comp, ( arg1 arg2 -- )
    2arg case
        dd' of sdreg $B000 + opsize+ asmh, endof
        ad' of nobyte sdreg $B008 + opsize+ asmh, endof
        md' of nip dreg $B000 + ea, endof
        #d' of nip dreg $B000 + opsize+ imm, endof
        da' of nobyte sdreg $B0C0 + opsize @ 4 and 6 lshift + asmh, endof
        aa' of nobyte adreg $B0C0 + opsize @ 4 and 6 lshift + asmh, endof
        ma' of nobyte nip dreg $B0C0 + 
               ea @ + opsize @ 4 and 6 lshift + asmh, ext1, endof
        #a' of nobyte nip dreg $B0C0 + opsize @ 4 and 6 lshift + imm, endof
        #m' of 2drop $0C00 ea @ + opsize+ asmh, imm#, ext1, endof
        mm' of 2drop --? not effect-error (easd) $B108 opsize+ asmh, endof
    invalid endcase clean ;  aka cmp,

: (csize+) ( n -- n' ) opsize @ 2 and 5 lshift + ;
: check, ( mem dreg -- )
    <word> 2arg case
        dd' of sdreg $4100 + (csize+) asmh, endof
        md' of nip dreg $4100 + (csize+) ea @ + asmh, ext1, endof
...
: move, ( arg1 arg2 -- )
    2arg case
        dd' of sdreg (size+) asmh, endof
        ad' of adreg (size+) asmh, endof
        da' of sdreg (size+) $40 + asmh, endof
        aa' of adreg (size+) $40 + asmh, endof
        md' of  nip dreg (size+) ea @ + asmh, ext1, endof
        dm' of drop sreg (size+) ea  (eadest) asmh, ext1, endof
        mm' of 2drop   0 (size+) ea' (eadest) ea @ + asmh, ext1, ext2, endof
        ma' of  nip dreg (size+) $40 + ea @ + asmh, ext1, endof
        am' of drop areg (size+) ea  (eadest) asmh, ext1, endof
        #d' of nip dreg over ?schar not nip opsize @ 4 <> or
               if (size+) imm, else swap $FF and + $7000 + asmh, endif endof
        #a' of  nip dreg (size+) $40 + imm, endof
        #m' of 2drop   0 (size+) ea (eadest) imm, ext1, endof
        ds' of      <status> sreg $44C0 + asmh, endof
        ms' of  nip <status> ea @ $44C0 + asmh, ext1, endof
        #s' of  nip <status>      $44C0   imm, endof
        sd' of swap <status> sreg $40C0 + asmh, endof
        sm' of drop <status> ea @ $40C0 + asmh, ext1, endof
    invalid endcase clean ;
...
: movep, ( arg1 arg2 -- )
    nobyte 2arg case
        dm' of drop (disp) dreg $0188 + (ugh) ea @ 7 and + asmh, ext1, endof
        md' of  nip (disp) dreg $0108 + (ugh) ea @ 7 and + asmh, ext1, endof
    invalid endcase clean ;

: (ctrl) ( n -- n' ) ea @ dup $38 and dup $18 = swap $20 = or effect-error + ;
:  pea, ( mem -- )
    1arg m' <> invalid-error drop $4840 (ctrl) asmh, ext1, clean ;
:  lea, ( mem areg -- )
    2arg ma' <> invalid-error nip dreg $41C0 + (ctrl) asmh, ext1, clean ;

: exg, ( reg reg -- )
    2arg case dd' of $F140 endof aa' of $F148 endof
              ad' of $F188 endof da' of swap $F188 endof invalid endcase
    swap sdreg + asmh, clean ;

: ext, ( reg ) nobyte 1arg d' <> invalid-error sreg $4880 + (ugh) asmh, clean ;

: clear, ( mem -- )
    1arg case
        d' of opsize @ 4 =
              if dreg $7000 else sreg $4200 opsize+ endif + asmh, endof
        m' of drop $4200 ea, endof
    invalid endcase clean ;  aka clr,

: link, ( n # areg -- ) 
    <long> 2arg #a' <> invalid-error sreg $4E50 + asmh, drop w imm#, clean ;
: unlink, ( areg -- )
    <long> 1arg a' <> invalid-error sreg $4E58 + asmh, clean ;  aka unlk,

: swap, ( dreg -- ) <long> 1arg d' <> invalid-error sreg $4840 + asmh, ;
: stop,  ( n # -- ) <word> } # { <> invalid-error $4E72 asmh, w imm#, ;
: trap,  ( n # -- ) <long> } # { <> invalid-error 15 and $4E40 + asmh, ;
: illegal,   ( -- ) <long> $4AFC asmh, ;
: reset,     ( -- ) <long> $4E70 asmh, ;
: nop,       ( -- ) <long> $4E71 asmh, ;
: ireturn,   ( -- ) <long> $4E73 asmh, ;  aka rte,
: return,    ( -- ) <long> $4E75 asmh, ;  aka rts,
: trapv,     ( -- ) <long> $4E76 asmh, ;
: ccreturn,  ( -- ) <long> $4E77 asmh, ;  aka rtr,

: branch?, ( disp cc -- )
    $6000 +cc <word> swap ?shalf not disp-error
    ?schar if $FF and + else swap asmh, endif asmh, clean ;
: branch, ( disp -- ) yes branch?, ;
: brasub, ( disp -- )  no branch?, ;

: decbra?, ( reg disp cc -- )
    <word> rot 1arg d' <> invalid-error
    sreg +cc $50C8 + asmh, w imm#, clean ;
: decbra, ( reg disp -- ) no decbra?, ;

: jump: ( opcode "name" -- ) create , does> ( mem -- )
    @ swap 1arg m' <> invalid-error drop (ctrl) asmh, ext1, clean ;
...
: movem, ( arg1 arg2 -- )
    nobyte 1arg m' = if
        ...
        (ugh) asmh, ext1, asmh, clean exit endif
    swap 1arg m' = if
...
: if ( cc -- cc orig ) $100 xor romsize 0 asmh, ;
: then  ( cc orig -- )

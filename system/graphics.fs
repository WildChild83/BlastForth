...
code (gfree1) ( vramaddr bytes -- vramaddr prev next bytes )
    DEBUG [IF]
        d2 peek, tos d1 move, z= if -267 # tos move, tp rpull, next, endif
        $FFFF000F # d1 and,  z<> if -267 # tos move, tp rpull, next, endif
        $FFFF000F # d2 and,
              z<> if cell # sp add, -269 # tos move, tp rpull, next, endif
    [THEN] d1 clear, d1 push, (gnode) [#] d1 h move, d1 push, next
code (gfree2) ( vramaddr prev next bytes -- vramaddr prev' next' bytes flag )
    d1 peek, d1 h test, z<> if [sp 10 +] d1 h compare, ult= if
        d1 [sp cell +] move, $FFFF # d1 and, 2 # d1 lsl, 2 # d1 h lsr,
        d1 swap, d1 video-ctrl [#] move, video-data [#] d1 move,
        d1 swap, d1 poke, tos push, tos clear, next,
...
( ---------------------------------------------------------------------------- )
: nametables ( -- vram-addr )
    foreground-address  background-address  min
    window-address min  sprite-address min  scroll-address  min ;
: init-graphics ( -- )
...
: (setup-plane-memory) ( -- )
    plane-size * 2* 8 kilobytes over - ?if
        over foreground-address + over free-video throw
        over background-address + swap free-video throw
    endif drop ;
: setup-graphics ( -- )
